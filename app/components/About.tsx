...
const pillars = [
  {
    icon: Ship,
    title: 'Global Reach',
    description: 'A vast network of shipping partners, ports, and carriers ensures your cargo moves seamlessly across Middle East, Asia, and every major trade route.',
  },
  {
    icon: Zap,
    title: 'Smart Technology',
    description: 'Real-time cargo tracking, AI-optimized routing, and a digital-first platform keep you in full control at every stage of your shipment.',
  },
  {
    icon: Leaf,
    title: 'Sustainability',
    description: 'We offset 100% of our carbon footprint and partner only with carriers committed to cleaner, greener freight shipping.',
  },
]

const milestones = [
  { year: '2008', event: 'Founded in Singapore with a vision to simplify global trade across Asia and the Middle East.' },
  { year: '2013', event: 'Expanded to 50 countries and launched our digital cargo tracking platform.' },
  { year: '2018', event: 'Reached 5,000 clients milestone and opened our Americas freight hub.' },
  { year: '2024', event: 'Operating in 150+ countries with 10,000+ monthly shipments across Middle East & Asia.' },
]

export default function About() {
  return (
    <section id="about" aria-label="About OceanAxis Shipping Company" className="py-16 sm:py-24 lg:py-32 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center mb-16 sm:mb-24 lg:mb-32">
                ...
                <div className="bg-white/10 backdrop-blur rounded-xl sm:rounded-2xl p-4 sm:p-6">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 mb-4 flex-wrap">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 bg-blue-400/20 rounded-xl flex items-center justify-center flex-shrink-0">
                      <Ship className="w-4 h-4 sm:w-5 sm:h-5 text-blue-300" aria-hidden="true" />
                    </div>
          ...
          <div className="space-y-5 sm:space-y-8 mt-8 lg:mt-0">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-navy-800 to-navy-600 bg-clip-text text-transparent leading-tight">
              Leading Freight & Shipping Company in Middle East & Asia
            </h2>
            <p className="text-base sm:text-lg text-navy-600 leading-relaxed">
              Founded in 2008, OceanAxis is a trusted international shipping and freight forwarding company serving businesses across the Middle East, Asia, and beyond. We built our logistics company around transparency, reliability, and technology.
            </p>
            <p className="text-base sm:text-lg text-navy-600 leading-relaxed">
              Today, we manage thousands of ocean freight, air freight, and land transport shipments each month, backed by 1,200+ logistics professionals and a proprietary tracking platform that gives clients full cargo visibility from warehouse to destination.
            </p>
            <ul className="space-y-3" aria-label="OceanAxis certifications and features">
              {[
                'ISO 9001:2015 Certified Freight Operations',
                'Real-time shipment tracking & alerts',
                'Dedicated account managers for every client',
                'Carbon-neutral shipping options available',
              ].map((point) => (
                <li key={point} className="flex items-start gap-3">
                  <CheckCircle className="w-5 h-5 text-navy-600 mt-0.5 flex-shrink-0" aria-hidden="true" />
                  <span className="text-navy-700 font-medium text-sm sm:text-base">{point}</span>
                </li>
              ))}
            </ul>
            <a href="#contact" className="btn-primary inline-flex items-center" aria-label="Contact OceanAxis shipping team">
              Work With Us
            </a>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 sm:gap-6 lg:gap-8 mb-16 sm:mb-24 lg:mb-32">
          {pillars.map((p, idx) => (
            <div key={idx} className="group p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-navy-100 hover:border-navy-300 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 bg-gradient-to-br from-white to-navy-50/50">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-navy-500 to-navy-700 rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform shadow-md">
                <p.icon className="w-6 h-6 sm:w-7 sm:h-7 text-white" aria-hidden="true" />
              </div>
        ...
        <div className="text-center mb-10 sm:mb-16">
          ...
          <p className="text-navy-600 max-w-xl mx-auto text-sm sm:text-base">
            From a small freight startup in Singapore to a global shipping leader serving Middle East & Asia.
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <div className="space-y-8 sm:space-y-12">
            {milestones.map((m, idx) => (
              <div key={idx} className={`flex items-start gap-4 md:items-center md:gap-8 ${idx % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
